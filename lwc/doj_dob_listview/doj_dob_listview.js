...
import getDob from '@salesforce/apex/updateCaseController.getDob';

const columns = [
      { label: 'Date of Birth', fieldName: 'Namelink', type: 'url', sortable: true, typeAttributes: { label: { fieldName: 'Name' }, tooltip: 'Go to detail page', target: '_blank' } },
    { label: 'Contact', fieldName: 'Contactlink', type: 'url', sortable: true, typeAttributes: { label: { fieldName: 'Dob__rname' }, tooltip: 'Go to offences', target: '_blank' } },
...
export default class Doj_dob_listview extends LightningElement {
   error;
    columns = columns;
        ...
        if (data) {
            ...
            for (let i = 0; i < data.length; i++) {
             ...
             record.Namelink = '/' + data[i].Id;
                record.Contactlink = '/' + data[i].DOBs__c;
                if (data[i].DOBs__r != undefined) {
                    record.Dob__rname = data[i].DOBs__r.Name;
                }
